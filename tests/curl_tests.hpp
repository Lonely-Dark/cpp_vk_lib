...
TEST(curl, POST_multithreaded)
{
    ...
    for (size_t i = 0; i < std::thread::hardware_concurrency(); ++i) {
        ...
        std::thread thread([promise = std::move(promise)]() mutable {
            ...
            if (received.error()) { FAIL() << "error while HTTP GET"; }
...
TEST(curl, POST_require_data)
{
    ...
    if (const auto result = received.value(); result.empty()) {
        ...
        if (result.find("Google") == std::string::npos) { FAIL() << "HTTP output mismatch"; }
    }
}
...
TEST(curl, GET_omit_data_speed_test)
{
    ...
    for (size_t i = 0; i < 5; ++i) { runtime::network::request(runtime::network::omit_data, url); }
}

TEST(curl, GET_require_data_speed_test)
{
    constexpr std::string_view url = "https://www.example.com";
    for (size_t i = 0; i < 5; ++i) { runtime::network::request(runtime::network::require_data, url); }
}

static std::string get_cat_url()
{
    static auto received =
        runtime::network::request(runtime::network::require_data, "https://api.thecatapi.com/v1/images/search");
...
{
    if (runtime::network::download("buffer", get_cat_url()) != 0) { FAIL() << "Failed to download to file"; }
    std::remove("buffer");
}
